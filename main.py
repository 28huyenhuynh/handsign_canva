...
Pipeline stages (timed individually for performance report):
  ...
  Stage 4 — Sign Classification (TFLite)
...
HOLD_FRAMES    = 20          # frames sign must be stable before action fires
CONF_THRESHOLD = 0.80        # minimum confidence to accept a prediction
COOLDOWN_SEC   = 1.5         # seconds to wait after firing before next action
TIMING_WINDOW  = 300         # frames to average for performance report
LOG_PATH       = "session_log.csv"

# ── Canva keyboard mapping ────────────────────────────────────────────────────
#   Each entry: sign_label → (description, pyautogui hotkey args)
SIGN_ACTIONS = {
...
def draw_hold_arc(img, cx, cy, progress):
    """
    Draws a circular progress arc showing how long the sign has been held.
    progress: 0.0 → 1.0
    """
    axes  = (32, 32)
    angle = int(360 * progress)
    cv2.ellipse(img, (cx, cy), axes, -90, 0, angle, CLR_GREEN, 4)
    cv2.ellipse(img, (cx, cy), axes, -90, angle, 360, (80, 80, 80), 2)
...
def main():
    ...
    # Stage timing buffers (deque auto-drops oldest beyond maxlen)
    ...
    pyautogui.FAILSAFE = False   # disable corner failsafe for live demos
    ...
    while True:
        ...
        _      = clahe.apply(gray)                # CLAHE (processed, not displayed here)
        ...
        # Prediction state for this frame
        pred_label = None
        pred_conf  = 0.0
        ...
        if hand_detected:
            ...
            # No hand — still record zeros for timing consistency
        ...
        if pred_label is not None:
            ...
            # Fire if held long enough AND cooldown has passed
            if (hold_count >= HOLD_FRAMES
                    and now - last_action_t >= COOLDOWN_SEC):
                action = SIGN_ACTIONS.get(pred_label)
                if action:
                    desc, fn = action
                    fn()                        # fire keyboard shortcut
                    last_action_t = now
        ...
        if pred_label:
            ...
            # Hold progress arc (top-right corner)
            progress = min(hold_count / HOLD_FRAMES, 1.0)
            draw_hold_arc(frame, w - 55, 45, progress)
            cv2.putText(frame, "HOLD",
                        (w - 72, 95),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.45, CLR_WHITE, 1)
        else:
            cv2.putText(frame, "No hand detected" if not hand_detected
                        else "Low confidence",
        ...
        if last_fired and now - last_fired_t < 1.5:
            desc = SIGN_ACTIONS[last_fired][0]
            draw_rounded_rect(frame, 10, 80, w - 10, 130,
                              CLR_GREEN, alpha=0.5)
            cv2.putText(frame, f"ACTION: {desc}",
                        (20, 115),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.75, CLR_WHITE, 2)
        ...
        if all(len(stage_times[s]) > 10 for s in range(1, 7)):
            ...
            for i, (s, lbl) in enumerate(zip(range(1, 7), labels_txt)):
                x = 10 + i * 105
                cv2.putText(frame, f"S{s} {lbl}",
                            (x, h - 85),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.38, CLR_AMBER, 1)
                cv2.putText(frame, f"{means[s]:.1f}ms",
                            (x, h - 68),
            ...
            cv2.putText(frame,
                        ...
                        (10, h - 45),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.55, CLR_GREEN, 1)
    ...
    for s in range(1, 7):
        if stage_times[s]:
            ...
            std  = np.std(arr)
            total_mean += mean
            print(f"  Stage {s}  {stage_names[s]:<26}"
                  f"  {mean:6.2f} ms  ±{std:5.2f} ms")
    print("-" * 58)
